...
export class StudentsDashboardComponent implements OnInit {
  ...
  //  view detail id
  viewDetail(id) {
    this.router.navigate(['/dashboard/course_detail_unpaid'], {
      queryParams: { id: id },
    });
  }
  ...
  studentCourseDetail(page = 1, i) {
    ...
    this.service.post('get-course', data, 1).subscribe((res) => {
      ...
      if (
        res.body.total_course === 1 ||
        res.body.total_course === 0 ||
        res.body.data[0].message == 'No Data Found'
      ) {
  ...
  enrollId(data, i) {
    ...
    if (
      ...
      this.router.navigate(['/dashboard/payment'], {
        queryParams: { url: this.enroll, id: data.id },
      });
    }
  }
  refresh() {
