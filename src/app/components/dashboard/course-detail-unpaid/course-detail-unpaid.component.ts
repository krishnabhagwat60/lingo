...
export class CourseDetailUnpaidComponent implements OnInit {
  ...
  courseDetail() {
    ...
    this.service.post('course-details', data, 1).subscribe((res) => {
      ...
      this.courseid = sessionStorage.setItem(
        'course_id',
        res.body.result.course_id
      );
      this.name = sessionStorage.setItem(
        'teacher_name',
        res.body.result.teacher_name
      );
      ...
      if (
        this.courseFaqdata == '' ||
        this.courseFaqdata == null ||
        this.courseFaqdata == undefined
      ) {
        this.fqButton = false;
      } else {
        this.fqButton = true;
  ...
  enrollId(data) {
    ...
    if (
      data?.field_course_fees.toLowerCase() == 'free' ||
      data.field_course_fees == '50 cent'
    ) {
      ...
      this.router.navigate(['/dashboard/payment'], {
        queryParams: { url: this.enroll, id: data.course_id },
      });
    }
  }
  submitEnrolls(amount) {
    ...
    this.frontServices.vm.courseSubmitted=true;
    this.service.post('course-enroll', data, 1).subscribe((res) => {
    this.frontServices.vm.courseSubmitted=false;
