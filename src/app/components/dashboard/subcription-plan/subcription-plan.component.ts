...
export class SubcriptionPlanComponent implements OnInit {
  ...
  timestamp: number;
  amount: any;
  constructor(private service: ServiceService, private router: Router, public datepipe: DatePipe) { }
  ...
  resubscribeId(data) {
    ...
    if (data.course_fee == 'Free' || data.course_fee == 'free' || data.course_fee == '50 cent') {
      this.submitEnroll()
      this.router.navigate(['/dashboard/wallet']);
    } else {
      this.submitEnroll()
      this.router.navigate(['/dashboard/payment'], { queryParams: { url: enroll, id: data.id } })
    }
  }
