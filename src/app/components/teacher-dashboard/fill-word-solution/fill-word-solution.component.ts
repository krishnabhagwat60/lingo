...
@Component({
  ...
  templateUrl: './fill-word-solution.component.html',
  styleUrls: ['./fill-word-solution.component.css']
})
export class FillWordSolutionComponent implements OnInit {
  fillData: any;
  totalQuestion: any;
  ...
  editFillBlanks() {
    ...
    this.service.post('paragraph-get', data, 1).subscribe(res => {
      this.fillData = res.body.result.fill_in_the_blanks;
      this.totalQuestion = res.body.result.total_question
      this.totalRightAnswer = res.body.result.total_right_answer
      this.fillData.forEach((element, index) => {
        if (element.question.includes('*')) {
