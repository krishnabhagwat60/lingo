...
export class StudentViewComponent implements OnInit {
  ...
  getQuestion() {
    ...
    this.service.post('question-dropdown-get', data, 1).subscribe(res => {
      this.questionDropDown = res.body.result,
        this.dropDataTitle = sessionStorage.setItem('question_title', res.body.result[0].title)
      if (res.body.result) {
  ...
  getTextDrag() {
    ...
    this.service.post('text-dragdrop-get', data, 1).subscribe(res => {
      ...
      this.textDataTitle = sessionStorage.setItem('text_title', res.body.result[0].title)
      if (res.body.result) {
        this.dragText = true;
      }
    })
  ...
  getImageDrag() {
    ...
    this.service.post('image-dragdrop-get', data, 1).subscribe(res => {
      ...
      this.imageDataTitle = sessionStorage.setItem('image_title', res.body.result[0].title)
      if (res.body.result) {
        this.dragImage = true;
      }
    })
  ...
  getRadio() {
    ...
    this.service.post('radio-button-get', data, 1).subscribe(res => {
      ...
      this.radioData = sessionStorage.setItem('radio_title', res.body.result[0].title)
      ...
      this.fillTheBlanksData.forEach((element, index) => {
        if (element.type === 'fill_in_the_blanks') {
          if (element.question.includes('*')) {
            element.question = this.findStarWord(element.question, index, element.p_id);
          }
        }
      });
    })
  ...
  pickGetData() {
    ...
    this.service.post('allexercises-get', data, 1).subscribe(res => {
      ...
      this.pickData = sessionStorage.setItem('pick_title', res.body.result.pick_up_right_word[0].title)
    })
  }
  ...
  findStarWord(word1, index, p_id) {
    ...
    for (let i = 0; i < array.length; i++) {
      var tag = '<input type="text" class="answer" id="answer' + p_id + index + '[' + i + ']' + '"' + '/>'
    ...
    this.fillTheBlanksData.forEach((element, index) => {
      if (element.type === 'fill_in_the_blanks') {
        for (var i = 0; i < this.arrayLength; i++) {
          if (element.p_id == id) {
            var currentValue = (<HTMLInputElement>document.getElementById("answer" + element.p_id + index + '[' + i + ']')).value;
  ...
  showResult(titleid, title) {
    ...
    this.router.navigate(['/teacherDashboard/affiliation-result'], { queryParams: { id: this.id, titleid: titleid } });

  }
  imageShowResult(titleid, title) {
    this.imageDataTitle = sessionStorage.setItem('image_title', title)
    this.router.navigate(['/teacherDashboard/image-drag-result'], { queryParams: { id: this.id, titleid: titleid } });
  }
  dragShowResult(titleid) {
    this.router.navigate(['/teacherDashboard/drag-word'], { queryParams: { id: this.id, titleid: titleid } });
  }
  questionRadioDropdown(titleid, title) {
    this.dropDataTitle = sessionStorage.setItem('question_title', title)
    this.router.navigate(['/teacherDashboard/question-dropDown-result'], { queryParams: { id: this.id, titleid: titleid } });
  }
  pickRightWord(titleid, title) {
    this.pickData = sessionStorage.setItem('pick_title', title)
    this.router.navigate(['/teacherDashboard/pick-right-word-result'], { queryParams: { id: this.id, titleid: titleid } });
  }
  fillResult(titleid) {
    // sessionStorage.setItem('fill_title')
    this.router.navigate(['/teacherDashboard/fill-result'], { queryParams: { id: this.id, titleid: titleid } });
  }
  showRadioResult(titleid, title) {
    this.radioData = sessionStorage.setItem('radio_title', title)
    this.router.navigate(['/teacherDashboard/questionradio-result'], { queryParams: { id: this.id, titleid: titleid } });
  }
